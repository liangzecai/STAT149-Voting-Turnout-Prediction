...
na.data$cd <- factor(na.data$cd)
na.data$hd <- factor(na.data$hd)
na.data$cd <- factor(na.data$cd)
na.data$hd <- factor(na.data$hd)
```
...
```{r}
tempData = mice(na.data,m=2,meth='rf',seed=0)
```
...
```{r}
train.nafill.X = completedData[1:118529, ]
test.nafill.X = completedData[118530:158039, ]
```

```{r}
## save the imputed data file
write.csv(train.nafill.X, file = "train_nafill_X.csv", row.names = FALSE)
write.csv(test.nafill.X, file = "test_nafill_X.csv", row.names = FALSE)
```
